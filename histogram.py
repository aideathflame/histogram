...
def main():
    ...
    report_lines.extend([
        ...
        "FAT TAILS ANALYSIS",
        "-" * 50,
        f"Jarque-Bera test: stat={jb_stat:.1f}, p={jb_pvalue:.4g}"
        f"  {'→ NOT normal' if jb_pvalue < 0.05 else '→ Cannot reject normality'}",
        "",
        ...
        f"Fat tail multiplier: {fat_tail_mult:.1f}x" if not np.isnan(fat_tail_mult) else "Fat tail multiplier: N/A (no expected outliers)",
    ])

    if not np.isnan(tail_ratio):
        report_lines.append(f"Tail ratio (neg/pos magnitude): {tail_ratio:.2f}x"
                            f"  {'→ downside dominates' if tail_ratio > 1.2 else '→ roughly symmetric' if tail_ratio > 0.8 else '→ upside dominates'}")
    else:
